...
index 354f5dde679b75d228cd7a3107355d36a8e83b70..7537b4321023475ed756e9de675dccc20953faaa 100644
--- a/run.py
+++ b/run.py
@@ -1,156 +1,178 @@
-import subprocess
...
-    time.sleep(1)
+    print(f"\nStarting simulation for scenario: {name}")
+    log_event(f"START SCENARIO | name={name} | target_successful_seeds={seeds}")
+    time.sleep(1)
...
+        print(f"Starting simulation for seed {cur_seed} (try {tries})...")
+        log_event(
+            f"START SIMULATION | scenario={name} | seed={cur_seed} | try={tries} | "
+            f"energy={energy} MeV | particle={particle} | histories={params['histories']}"
+        )
+        time.sleep(2)
